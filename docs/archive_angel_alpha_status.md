...
- Rows follow catalog renames (Rick 2026-09-10: "Show in Catalog, rename, come back to AA and see the refreshed name"): `ArchiveAngelPlan.follow` (pure) + `ArchiveAngelPromoter.followRenames` run on Archive-tab refresh, sheet open and Promote. Followed only when the file at the record's new path is the prepared one (size + content hash); a rewritten file is refused with the new path named. The proposed archive name tracks the new stem unless the user typed one in the sheet (`userEditedName`).
