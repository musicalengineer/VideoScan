...
## Amendment after codex review #984 (2026-09-02) — ACCEPTED, supersedes amendment 2's "proposal" reading
10. **One sibling policy, everywhere.** Rick's ruling (2026-09-02, "full siblings share parents") is implemented ONCE, in `FamilyKinshipOverlay.deriveSharedParents`, and every reader — `FamilyKinshipInference`, `KinshipValidation`, `KinshipDisplayCenter`, `HallieRelationshipsOverview`, the biography card and the kinship route — consumes the overlay's derived edges. An `.unspecified` sibling row is a FULL sibling (both parents shared, propagating through the sibling set); `.attestedFull` is the same; `.attestedHalf(sharedParent:)` shares only its named parent and DOMINATES any unspecified / reciprocal row for the same unordered pair. Rows are coalesced per unordered pair before union. A full-vs-half conflict for one pair, more than two implied parents for a set, two parents of the same recorded sex, or a member who is also a parent of the set FAILS CLOSED: nothing is derived for that set and one warning is delivered to every involved profile (`warnings(forProfileNamed:)`) and to Hallie's basis (`derivationWarnings(touching:)`); the checks run whether or not anyone is missing a parent. A sibling with one stored parent receives the set's other parent (per-parent merge); a stored parent is never replaced. Derived edges carry the SOURCE row's profile identity (the card the row sits on), never the child's or the parent's. `proposals(for:)` now only offers to WRITE a derivation onto the sibling's card; the engine's "not attested" route and caveats are gone. Wording shared by every surface: "derived: full siblings share parents" / "half siblings share the named parent".
