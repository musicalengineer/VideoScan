...
- **Allowed files:** `ArchiveAngelScorer.swift`, `ArchiveAngelScorerTests.swift`, `ArchiveAngelCandidate+Record.swift` (projection only, additive), `docs/archive_angel_design.md`. Anything else → GH issue.
  - **Scope expansion declared 20:45 ET (owner authority, logged; codex #1297):** `ArchiveAngelJob+Evidence.swift` `selectFromEvidence` only — the production pick path bypasses `ArchiveAngelScorer.select`, so a pick-time rule (H2 one-per-duplicate-group) that lives only in `select` would never reach a fresh-evidence batch. The change is the same five-line rule in both paths plus `ArchiveAngelEvidencePickTests`. No other file. The reviewer may still block H2 on this expansion; a block parks H2 as an issue.
  - **Second expansion declared (H3, same authority):** one call line each in `VideoScanModel+ArchiveAngelSweep.swift` (`archiveAngelSweepCandidates`) and `ArchiveAngelJob.swift` (walk path) invoking the pure, in-scope `ArchiveAngelScorer.markDerivatives(&candidates)` — a derivative needs the whole candidate set to know its original is present, and both producers must agree. No logic in either file.
...
- **Adversarial acceptance (codex 2026-09-10), each heuristic's tests must show:** (1) genuinely old family footage with NO camera metadata is retained (a 1984 DV capture with a tape name but no embedded date; a 1990s .mov with a folder-year only); (2) duplicate collapse acts only inside a real `duplicateGroupID`, never across unknown-group records, and keeps the best eligible member (highest score, then longest, then largest); (3) suffix demotion requires an actual related original present and eligible (same stem before the suffix, same folder or same duplicate group), never a bare filename suffix or an unrelated basename match. The morning brief lists genuine-family candidates RETAINED beside unwanted ones removed.
- **Night result (21:00 ET):** H1 (download/rip cap + human-note classifier + family-origin folder exemption) cleared by codex and merged. H2 (one per duplicate group) and H3 (derivative exports) were built on `fix/angel-hygiene-1` (kept, unmerged) and PARKED on reviewer blockers — both needed call sites outside the allowed files, and the reviewer found rule defects (equal-score group member order on the evidence path; derivative fallback to a same-named original in any folder without checking the original is eligible). Filed as issues; scope expansion is Rick's call, not the owner's.
...
- **Checkpoint review precedes the merge for themed work** (this supersedes Night protocol item 5's review-after-merge for themes with a reviewer on duty; item 5 remains the rule for unreviewed daytime merges).
