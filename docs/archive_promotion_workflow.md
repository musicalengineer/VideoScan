...
- Date source (2026-08-16, `RecordDateResolver` — the ONE shared ranking for placement and readiness): (1) Rick's `userDate` at its precision — a year/month-only user date is refined, never overruled, by an agreeing finer machine date; (2) `embeddedCreationDate` (the container's own `creation_time` / `com.apple.quicktime.creationdate` / stream stamp, sanity-filtered; confidence 0.95 when the origin names a camera/phone make/model, 0.80 when only a transcoder encoder is named, 0.85 otherwise); (3) `inferredRecordDate` + confidence ≥ 0.6; (4) a date pattern in the filename (YYYY-MM-DD, MM-DD-YYYY, YYYYMMDD, "Month YYYY", YYYY-MM, or a bare 19xx/20xx year → year precision) at 0.5, placed but flagged low-confidence; (5) Undated. Filesystem dates are never used. Promotion **warns**, never blocks, on Undated/low-confidence.
...
- **Inspector**: "Master copy ✓ · Reveal" on sources; "Promoted from … · Reveal source" on archive copies. Show menu: **Not Yet Archived** / **Has Master Copy** filters. *Not Yet Archived* hides promoted copies, sources that have a master copy, **and anything living inside the Master Archive root** (2026-09-11 — older `.vs.*` versions written straight into the archive folders used to linger in the to-do view while Promote refused them). The Tag column shows the Archive Angel grade as a badge: **Promote me** (A) / **Worth a look** (B). A *version* of something archived (balanced audio, trim, older transcode — anything up the `derivedFrom` chain that is archived) is hidden too; repairs (external repair, rebuilt audio) stay visible. Right-click → **Prepare with Archive Angel** hands the selection straight to the Angel (same buffer + review sheet).
