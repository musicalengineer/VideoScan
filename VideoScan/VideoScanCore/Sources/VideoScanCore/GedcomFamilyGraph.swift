...
public struct GedcomFamilyGraph: Sendable {
    ...
    struct Family: Sendable {
        ...
        /// FamilySearch's stable family identifier (`1 _FSFTID` under a
        /// FAM record). A family FamilySearch itself knows outranks a
        /// stray local one when a person carries two FAMC links (Rick's
        /// 2026-09-02 ruling; see GedcomFamilyGraph+ParentFamily).
    ...
    public func relatives(_ relation: Relation, of person: Person) -> [Person] {
        ...
        switch relation {
        // One father and one mother — the PRIMARY parent family's (Rick,
        // 2026-09-02: "pick one and not list two Moms for one person").
        // A second FAMC (a FamilySearch duplicate, or a genuine adoptive /
    ...
    private func parentFamilies(of person: Person) -> [Family] {
        parentFamilyIDs(of: person).compactMap { families[$0] }
    }

    /// Every FAMC pointer the record carries, in file order (the single
    /// `childOfFamily` when the list is empty — older decoded records).
    func parentFamilyIDs(of person: Person) -> [String] {
        person.childOfFamilies.isEmpty
            ? [person.childOfFamily].compactMap { $0 }
            : person.childOfFamilies
    }

    /// Every recorded parent across EVERY parent family — fathers then
    /// mothers, FAMC order, no repeats: the pre-ruling union. For audits
    /// (FamilyTreeVerification) that must still see a duplicate or a step
    /// family; never for prose.
    public func allRecordedParents(of person: Person) -> [Person] {
        let families = parentFamilies(of: person)
        return uniquePeople(families.compactMap { $0.husband.flatMap { people[$0] } }
            + families.compactMap { $0.wife.flatMap { people[$0] } })
    }

    func uniquePeople(_ candidates: [Person]) -> [Person] {
        var seen: Set<String> = []
    ...
    static let keptFamilyTags: Set<String> = ["HUSB", "WIFE", "CHIL", "MARR", "_FSFTID"]
    ...
    private static func applyFamilyLine(
        ...
        case (1, "_FSFTID") where family.familySearchID == nil:
            let identifier = value.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
            guard isFamilySearchID(identifier) else { return false }
            family.familySearchID = identifier
