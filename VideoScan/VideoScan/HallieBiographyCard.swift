...
enum HallieBiographyCard {
    ...
    struct Card: Sendable, Equatable {
        ...
        /// family record, or on one side only (codex #1011): the sibling
        /// sentence lists full siblings — the ONE sibling verdict's — and
        /// this note carries the rest to the basis line, in the same words
        /// the kinship route uses, so the two never disagree.
