...
final class DashboardState: ObservableObject {
    ...
    private static func detectChipName() -> String {
        ...
        guard !s.isEmpty else { return "Apple Silicon" }
        return s + chipDetailSuffix(chipName: s)
    }

    /// " · 64 GB · 16 CPU (12P+4E) · 40 GPU · 16 ANE" — each segment
    /// appended only when the machine actually answers for it, so an
    /// Intel Mac degrades to name + RAM + CPU with no fake zeros.
    private static func chipDetailSuffix(chipName: String) -> String {
        var parts: [String] = []
