...
enum ArchivistPresenceAnswerComposer {
    ...
    static func parseFacets(_ interpretedQuery: String) -> ParsedFacets {
        ...
        // Last guard before ANY prose built from these facets: a name is
        // never said twice in two casings (demo eval lv260902-004 —
        // "videos tagged with tim and Tim"). Applied here rather than in
        // noEvidenceAnswer so the relaxed OFFER, which parses the same
        // facets, cannot reintroduce the duplicate by another road.
        facets.people = PersonNameClaim.dedupe(facets.people)
        return facets
    }
