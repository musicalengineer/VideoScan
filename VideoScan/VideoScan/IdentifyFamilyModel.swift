...
final class IdentifyFamilyModel: ObservableObject {
    ...
    enum PromotionAction: Identifiable {
        /// A new person; `uuid` is minted at plan time and is the folder the
        /// faces land in, so plan and apply cannot disagree.
        case create(clusterID: Int, name: String, faceCount: Int, uuid: UUID)
        /// An existing person, resolved ONCE at plan time (canonical name,
        /// else a unique alias / full-name form) and carried by uuid.
        case merge(clusterID: Int, name: String, faceCount: Int, existingCount: Int, uuid: UUID)
        case skip(clusterID: Int, reason: String)

        var id: Int {
            switch self {
            case .create(let cid, _, _, _), .merge(let cid, _, _, _, _), .skip(let cid, _): return cid
            }
        }
    }

    /// Who a cluster name means among the saved profiles: exact canonical
    /// name wins (PersonNameClaim's rule); otherwise the unique profile whose
    /// alias or full-name form is the typed spelling ("Rick" → Richard Jr).
    /// Two claimants at the winning strength are ambiguous — never guessed.
    ...
    static func resolveProfile(named name: String, in profiles: [POIProfile]) -> NameResolution {
        let key = PersonResolver.normalize(name)
        guard !key.isEmpty else { return .none }
        let byName = profiles.filter { PersonResolver.normalize($0.name) == key }
        if !byName.isEmpty {
            return byName.count == 1 ? .one(byName[0]) : .ambiguous(byName)
        }
        let byAlias = profiles.filter { profile in
            (profile.aliases + profile.fullNameForms).contains { PersonResolver.normalize($0) == key }
        }
        switch byAlias.count {
        case 0: return .none
        case 1: return .one(byAlias[0])
        default: return .ambiguous(byAlias)
        }
    }
    ...
    func planPromotion() -> [PromotionAction] {
        ...
        for c in clusters where c.id != -1 {
            ...
            switch Self.resolveProfile(named: trimmed, in: profiles) {
            case .ambiguous(let owners):
                plan.append(.skip(clusterID: c.id,
                                  reason: "\(owners.count) people answer to \(trimmed) — merge by hand"))
            case .one(let owner):
                ...
                plan.append(.merge(
                    clusterID: c.id, name: owner.name,
            ...
            case .none:
                plan.append(.create(
                    clusterID: c.id, name: trimmed, faceCount: c.faceCount, uuid: UUID()
                ))
            }
        }
    ...
    func executePromotion(_ plan: [PromotionAction]) -> String {
        ...
        for action in plan {
            ...
            case .skip:
                continue
            case .create(let cid, let name, _, let uuid), .merge(let cid, let name, _, _, let uuid):
                guard let cluster = clusters.first(where: { $0.id == cid }) else {
                    log.error("Cluster \(cid) not found in clusters array — skipping")
                    continue
                }
                // The plan carries the uuid: an existing person's folder for
                // a merge, the minted one for a create. Nothing is resolved
                // by name again here.
                let isCreate: Bool
                if case .create = action { isCreate = true } else { isCreate = false }
                var profile: POIProfile
                if isCreate {
                    profile = POIProfile(name: name, referencePath: "", uuid: uuid)
                    profile.referencePath = POIStorage.folder(for: profile).path
                } else {
                    guard let existing = profiles.first(where: { $0.uuid == uuid }) else {
                        log.error("Cluster \(cid): profile \(uuid.uuidString, privacy: .public) vanished since the plan — skipping")
                        continue
                    }
                    profile = existing
                }
