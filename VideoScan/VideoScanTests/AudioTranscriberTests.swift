...
struct AudioTranscriberTests {
    ...
    //
    // NOTE: The previous `subprocessEnvironmentIncludesHomebrewPaths`
    // test moved to SubprocessEnvTests.swift when the helper itself
    // moved to a shared file (fix/identify-family-path). The contract
    // is unchanged — it's just tested at the helper's home now.
