...
struct HallieRelativeFactTests {
    ...
    @Test func grandmotherClarificationRetainsBirthplaceAndStableIdentity() async throws {
        ...
        #expect(HallieTurnExecutor.clarificationSelection("the one from Cork", from: pending.candidates) == maternal.id)
        #expect(HallieTurnExecutor.clarificationSelection("Dad's mother", from: pending.candidates) == paternal.id)
    ...
    @Test func sideAndGreatGrammaResolveWithoutCatalog() async throws {
        ...
        #expect(paternal.basisLine.contains("paternal grandmother"))
