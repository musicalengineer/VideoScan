...
struct CatalogStoreAsyncSaveTests {
    ...
    func snapshotDTOEncodesGoldenJSON() throws {
        ...
        let produced = String(decoding: try dtoSnapshot.encoded(using: Self.goldenEncoder()),
                              as: UTF8.self)
        #expect(produced == Self.snapshotGolden,
                "CatalogSnapshotDTO drifted from the catalog.json wrapper golden — see step 5b guard")
        // GH #165: the wrapper's key order is fixed by hand, NOT by the
        // encoder — `version` then `generation` lead, `records` trails —
        // even though the golden encoder asks for sortedKeys (which still
        // governs the record objects, as the embedded record golden proves).
        #expect(produced.hasPrefix(#"{"version":6,"generation":0,"savedAt":"2025-06-15T15:06:40Z","savedFromHost":"MacStudio","records":["#))
        #expect(produced.hasSuffix("]}"))
    }
